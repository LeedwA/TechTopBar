...
public class BarLollipop {
    ...
    public static void setStatusBarWhiteForCollapsingToolbar(final Activity activity, final AppBarLayout appBarLayout, final CollapsingToolbarLayout collapsingToolbarLayout, final Toolbar toolbar, final int statusBarColor) {
        final Window window = activity.getWindow();
        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        window.setStatusBarColor(Color.TRANSPARENT);
            ...
            public void onOffsetChanged(AppBarLayout appBarLayout, int verticalOffset) {
                ...
                if (Math.abs(verticalOffset) > collapsingToolbarLayout.getScrimVisibleHeightTrigger()) {
                    if (appBarLayoutState != COLLAPSED) {
                        ...
                        if (BarUtil.MIUISetStatusBarLightMode(activity, true)) {
                            return;
                        }
                        ...
                        } else if (!BarUtil.FlymeSetStatusBarLightMode(activity, true)) {
                    ...
                    if (appBarLayoutState != EXPANDED) {
                        ...
                        if (BarUtil.MIUISetStatusBarLightMode(activity, false)) {
                            translucentStatusBar(activity, true);
                            return;
                        }
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
